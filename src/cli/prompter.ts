...
/** Parse a typed approval answer. Empty input denies (safer default). */
export function parseApprovalAnswer(raw: string): ApprovalAnswer {
...
export function createInteractivePrompter(color: boolean): Prompter {
  ...
  return async (request): Promise<ApprovalAnswer> => {
    ...
    try {
      ...
      const answer = await rl.question(c.bold('[y]es [n]o [a]lways [e]dit > '));
